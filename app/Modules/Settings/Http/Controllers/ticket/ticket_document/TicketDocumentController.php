...
class TicketDocumentController extends Controller
{
    ...
    public function sendMailStore(Request $request,$id){
        ...
        Mail::send('settings::order.order.mail',array('order'=>$email),function($messege) use($result){

            $messege->to(Input::get('email_address'))->subject('Welcome to Ontik Technology');
