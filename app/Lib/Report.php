...
class Report
{
      ...
      public function OperatingincomeTotal($id)
      {
        if(is_null($this->start) && is_null($this->end)){
            $debt= JournalEntry::where('account_name_id',$id)->where('debit_credit',0)->whereYear('created_at',date('Y'))->sum('amount');
            $crt= JournalEntry::where('account_name_id',$id)->where('debit_credit',1)->whereYear('created_at',date('Y'))->sum('amount');
            $total = (int)$debt-$crt;
            $this->OperatingincomeTotal = $this->OperatingincomeTotal+$total;
            return $total;
        }else{
            $debt= JournalEntry::where('account_name_id',$id)->where('debit_credit',0)->whereBetween('created_at',array($this->start,$this->end))->sum('amount');
            $crt= JournalEntry::where('account_name_id',$id)->where('debit_credit',1)->whereBetween('created_at',array($this->start,$this->end))->sum('amount');
    ...
    {
        if(is_null($this->start) && is_null($this->end)) {
            $debt = JournalEntry::where('account_name_id', $id)->where('debit_credit', 1)->whereYear('created_at', date('Y'))->sum('amount');
            $crt = JournalEntry::where('account_name_id', $id)->where('debit_credit', 0)->whereYear('created_at', date('Y'))->sum('amount');
            $total = (double)$debt - $crt;

            $this->CostofGoodTotal = $this->CostofGoodTotal + $total;
            return $total;
        }else{
            $debt = JournalEntry::where('account_name_id', $id)->where('debit_credit', 1)->whereBetween('created_at',array($this->start,$this->end))->sum('amount');
            $crt = JournalEntry::where('account_name_id', $id)->where('debit_credit', 0)->whereBetween('created_at',array($this->start,$this->end))->sum('amount');
            $total = (double)$debt - $crt;
