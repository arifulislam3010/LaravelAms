...
class BankReport
{
    ...
    public function deposit($account_id=null)
  {
     ...
     $dep = JournalEntry::where('debit_credit',1)->where('account_name_id',$account_id)->whereBetween('created_at',array($this->start,$this->end))->sum('amount');
     return $dep;
  }
    ...
    public function withdraw($account_id=null)
    {
        ...
        $withdraw =JournalEntry::where('debit_credit',0)->where('account_name_id',$account_id)->whereBetween('created_at',array($this->start,$this->end))->sum('amount');
        return $withdraw;
    }
