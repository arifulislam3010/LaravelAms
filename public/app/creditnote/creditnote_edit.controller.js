...
function CreditNoteController($scope, $http) {
    ...
    $http
        .get(window.location.origin + "/api/credit-note/items/" + cn_id, {
            transformRequest: angular.identity,
            headers: {'Content-Type': undefined, 'Process-Data': false}
        })
        .then(function(response){
            $scope.credit_notes = response.data.credit_note_entries;
            item = response.data.items;
            tax = response.data.taxes;
            account = response.data.acounts;
            var cn = response.data.credit_note;
            $scope.adjustment = cn.adjustment;
            $scope.shipping_charge = cn.shiping_charge;
        })
        .finally(function () {
            $scope.total = 0.00;
    ...
    $http
        ...
        .then(function(response){
            ...
            item = response.data.items;
            tax = response.data.taxes;
            account = response.data.accounts;
            ...
            angular.forEach($scope.credit_notes, function(credit_note) {
                ...
                console.log(credit_note.amount);
